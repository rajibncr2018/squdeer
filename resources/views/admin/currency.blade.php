      ...
      <div class="span12">
        ...
        <div class="widget-box">
          <div class="widget-title"> <span class="icon">
            ...
            <h5><?=$title;?></h5>
            <a href="" class="delete-multiple-row" id="currency,currency_id"><span class="label label-info" style="border-radius: 3px; background: #bd362f;">Delete</span></a>
            <a href="{{url('admin/add-currency')}}"><span class="label label-info" style="border-radius: 3px; background: #04c;">Add New</span></a>
          </div>
          <div class="widget-content nopadding" id="tabledata">
            <table id="mytable" class="table table-bordered table-striped with-check data-table">
              ...
              <tbody>
                ...
                ?>
                <tr>
                  <td><input type="checkbox" name="multipleid[]" value="<?=$value->currency_id;?>" class="delete-multiple" /></td>
                  <td><?=$value->currency;?></td>
                  <td style="text-align: center;"><span id="statusid<?=$value->currency_id;?>"><a class="btn btn-mini change-status" id="<?=$value->currency_id;?>,currency,is_blocked,<?=$value->is_blocked;?>,currency_id" href="javascript:void(0)"><?=$value->is_blocked==0 ? '<i class="icon-ok" aria-hidden="true" style="color:#17C200"></i> Active' : '<i class="icon-remove" aria-hidden="true" style="color:#FF1F1F"></i> Inactive';?></a></span>  | <a class="btn btn-mini" href="{{url('admin/add-currency/'.base64_encode($value->currency_id))}}"><i class="icon-edit" style="color: #4537C8;"></i> Edit</a> | <a href="javascript:void(0)" class="btn btn-mini delete-one" id="<?=$value->currency_id;?>,currency,currency_id"><i class="icon-trash" <i class="icon-trash" style="color: #FF1F1F;"></i> Delete</a></td>
                </tr>
                <?php
                }
                ?>
